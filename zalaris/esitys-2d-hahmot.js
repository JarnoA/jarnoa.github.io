...
const HAHMOKERROKSET = [
  ...
  // silti täsmälleen keskellä juuri kun kettu on paikalla.
  {
    ...
    x: 1290,
    siirtymaX: -75,
    video: 'assets/2d/video/god-transparent.webm',
    ekaFrame: 'assets/2d/video/god-transparent-eka.png',
    viimeinenFrame: 'assets/2d/video/god-transparent-viimeinen.png',
  ...
  { x: 12240, aaniraita: 'Sounds/huomio.mp3' },
  ...
  // JO OLEMASSA OLEVAAN slow.png-kiintea-kuvaan, ei uutta erillistä alkiota.
    { x: 12690, kuva: 'kuvat/slow.png', kiintea: true, puoli: 'keski', pysty: 'ala', korkeusProsentti: 0.5, syvyysSuhde: 0.35, aaniraita: 'Sounds/hitaus.mp3'  },
  ...
  { x: 1300, aaniraita: 'Sounds/neiluskonto.mp3' },
  { x: 2630, aaniraita: 'Sounds/media3-91.mp3' },
  ...
  { x: 2190, kuva: 'kuvat/sokrates.png', korkeusProsentti: 0.74, scrollFactor: 2.1, syvyysSuhde: 0.45, yProsentti: 0.5, ankkuri: { x: 0.5, y: 0.0 }, aaniraita: 'Sounds/sokrates.mp3' },
  ...
  { x: 7440, aaniraita: 'Sounds/manipulointi.mp3' },

  // x:7740 "Tekoälyalustojen taloudellisten taustojen ymmärtäminen..." -
  // Trump-kuva kiinteänä + Valkoinen talo maailmaan-ankkuroituna taustalla,
  // kuvittamaan tekoälyalustojen poliittis-taloudellista taustaa.
  ...
  { x: 8790, aaniraita: 'Sounds/sosiaalinenmedia.mp3' },
  ...
  // Kuvat()-kommentti, sama mekanismi kuin torninilla (x:6390) oli jo.
  {
    ...
    yProsentti: 0.96, // 6.9.2026, Jarno: "it is floating, bring it to bottom" (0.82->0.90->0.96)
    scrollFactor: 0.50,
    syvyysSuhde: 0.45,
    vainPysahdys: 11417,
  },
  ...
  // ääniraita, ei omaa kuvaa/videota.
  { x: 6240, aaniraita: 'Sounds/neillawrence.mp3' },
  ...
  // ==========================================================================
  // PIILOTETUT ESINEET - asiakasyritysten tuotteet OSANA MAISEMAA
  // (8.9.2026, Jarno: "add hidden items to background, on positive slides...
  // it would be nice if their products would be visible"). Zalariksen yleisö
  // tulee kymmenestä nimetystä asiakasyrityksestä (ks. esitys-data.js x:93):
  // Metsä Group, Stora Enso, Finnair, DNA, Gigantti, Ericsson, Siemens,
  // Santander, Danske Bank ja UPM (uusin asiakas). EI logoja - geneerinen,
  // brändäämätön esine joka VIITTAA tuotteeseen. Tarkoitus on pieni ilo
  // tarkkaavaiselle katsojalle, EI sponsoripalkki.
  //
  // TÄMÄ ON TOINEN TOTEUTUS. Ensimmäinen (samana päivänä, poistettu) käytti
  // #piilotettuEsine-DOM-elementtiä: position:fixed -ikoni (emoji + yksi SVG)
  // joka häivytettiin ruudun nurkkaan. Se EI toiminut, kahdesta syystä jotka
  // molemmat on korjattu tässä:
  //   1) MEKANISMI. Jarno: "my idea was that they would be part of
  //      backgrounds, scenes, not like small objects every now and there."
  //      Ruutuun naulattu elementti ei skrollaa eikä parallaksoi maailman
  //      mukana -> lukee HUD-ikonina. Nämä ovat MAAILMAAN ankkuroituja
  //      HAHMOKERROKSET-alkioita, sama mekanismi kuin Tampella-siluetilla
  //      (x:11417) ja Valkoisella talolla (x:7740) - ne liikkuvat kohtauksen
  //      omassa parallaksissa ja jäävät lähempien kerrosten TAAKSE.
  //      Finnairin kone erityisesti: aiempi versio oli ✈️-emoji jota
  //      liikutettiin CSS-keyframellä maailmasta riippumatta ("plain did not
  //      look at all like flying plane") - nyt se on oikea konesiluetti
  //      taivaalla, ja sen liike tulee KOKONAAN kohtauksen parallaksista.
  //   2) TYYLI. Jarno: "items does not feel like 2d pixel art." Emojit ja
  //      sileä vektori-SVG lukivat käyttöliittymäikoneina. Nämä kymmenen
  //      assettia on piirretty natiivisti pienessä resoluutiossa (14-44 px
  //      korkeita), kovilla pikselireunoilla ja kunkin OMAN KOHTAUKSEN
  //      taustakuvista poimitulla vaimealla paletilla - ks. generaattori
  //      materiaalia/tee-piilotetut-esineet.py (ajettavissa uudelleen).
  //      Phaserin pixelArt:true (NEAREST) suurentaa ne 3.4-5.1-kertaisiksi
  //      ILMAN pehmennystä, joten pikselit jäävät näkyviin.
  //
  // lukitseKerrokseen (EI käsin arvattu scrollFactor): jokainen esine on
  // lukittu SEN OMAN kohtauksen tiettyyn taustakerrokseen, jolloin moottori
  // laskee ruutukorkeuden-tietoisen tehollisen scrollFactorin ajonaikaisesti
  // - sama korjaus jolla linnan "different phase" -bugi ratkaistiin 3.9.2026
  // (ks. esitys-2d.html:n laskeLukittuScrollFactor()). Käsin kirjoitettu
  // scrollFactor EI ole verrannollinen kerrosten scrollFactoreihin.
  //
  // syvyysSuhde on aina VALITUN KERROKSEN INDEKSI + 0.5 jaettuna kohtauksen
  // kerrosmäärällä: esine piirtyy siis TÄSMÄLLEEN lukitun kerroksen ETEEN
  // mutta seuraavan (lähemmän) kerroksen TAAKSE. Juuri tuo osittainen
  // peittyminen on vahvin vihje siitä että esine on maisemassa SISÄLLÄ eikä
  // sen päällä liimattuna.
  //
  // x EI ole sama kuin pysähdyksen x: ruutusiirtymä keskeltä on
  // (h.x - pysähdyksen x) * tehollinen_scrollFactor * SKAALA, joten esine
  // työnnetään sivuun TEKSTIN VASTAKKAISELLE puolelle (puoli-kenttä
  // esitys-data.js:ssä) antamalla x hieman pysähdyksen x:stä poikkeavaksi.
  // vainPysahdys on aina PYSÄHDYKSEN oma x - se ratkaisee näkyvyyden.
  // Kaikki x-arvot on tarkistettu pysyvän oman kohtauksensa xAlueella.
  //
  // Jarno: "do not add any to kiitos page" - loppudia (x:13890) on tyhjä.
  // Zalaris-kohtainen (asiakaslista on Zalariksen), EI viedä SectoDesignin
  // eikä Taiteilijajärjestöjen esityksiin.
  // ==========================================================================

  // FINNAIR - matkustajakone taivaalla. Pysähdyksen teksti on oikealla, kone
  // siis vasemmalla ja korkealla. Lukittu metsän KAUKAISIMPIIN kerroksiin
  // (Layer_0006_4), jolloin se liikkuu hitaasti kuten kaukainen kohde ja
  // lähemmät latvustot kulkevat sen EDESTÄ - juuri se "lentää puiden takana"
  // -vaikutelma jota ruutuun naulattu emoji ei voinut antaa.
  // POIKKEUS syvyyssääntöön (mitattu selaimessa 8.9.2026): kone oli aluksi
  // syvyysSuhde 0.278 (= lukitun kerroksen indeksi + 0.5, sama sääntö kuin
  // muilla) ja oli silloin TÄYSIN näkymätön. Syy: metsäkohtauksen kerrokset
  // 3-8 EIVÄT ole siluetteja joissa taivas näkyy läpi (kuten kaupunki-
  // kohtauksissa) vaan täyskorkeita, ruudun yläosassa läpinäkymättömiä
  // latvusmassoja - ne peittivät koko taivasalueen. 0.722 (= syvyys 6.5)
  // nostaa koneen niiden eteen mutta jättää sen yhä kahden lähimmän
  // etualan puukerroksen taakse. Tarkistettu silmällä: kone näkyy.
  // x siirretty 310 -> 184 samalla, koska 310:llä kone osui otsikon
  // "Jokainen..." vasempaan reunaan (teksti on tällä pysähdyksellä oikealla).
  // omaLiike (8.9.2026, Jarno testasi: "Plaine could be flying"): kone
  // lentää hitaasti eteenpäin (nokka osoittaa oikealle) niin kauan kuin
  // tämä pysähdys on näkyvissä. 16 px/s = noin kolmasosa ruudun leveydestä
  // puolessa minuutissa - matkalentokoneen tuntuma kaukana taivaalla, ei
  // ohi suhahtava lentonäytös. Ks. update():n omaLiike-lohko: liike menee
  // MAAILMAKOORDINAATTIIN, joten parallaksi vaikuttaa siihen edelleen.
  { x: 184, kuva: 'kuvat/piilotetut/finnair-lentokone.png',
    lukitseKerrokseen: 'Layer_0006_4.png', syvyysSuhde: 0.722,
    yProsentti: 0.12, ankkuri: { x: 0.5, y: 0.5 },
    korkeusProsentti: 0.05, omaLiike: 16, vainPysahdys: 540 },

  // SIEMENS - sykemonitori, jonka EKG-käyrä juoksee (Siemens Healthineers,
  // Jarno: "gear is boring, maybe healthcare some monitor, heart").
  // SPRITESHEET, 8 kehystä, hidas fps - rauhallinen pulssi, ei vilkkuva
  // huomiomagneetti. Teksti vasemmalla -> monitori oikealla, maan tasolla
  // puiden seassa.
  { x: 3040, spritesheet: 'kuvat/piilotetut/siemens-monitori.png',
    kehysLeveys: 26, kehysKorkeus: 22, kehyksia: 8, fps: 6,
    lukitseKerrokseen: 'Layer_0003_6.png', syvyysSuhde: 0.611,
    yProsentti: 0.80, korkeusProsentti: 0.075, vainPysahdys: 2940 },

  // STORA ENSO - pahvilaatikko. TÄMÄ pysähdys on erikoistapaus: sillä on
  // kupla:true eli keskitetty kuplaefekti (ks. esitys-data.js x:8940), joten
  // laatikko on työnnetty selvästi vasemmalle JA matalalle maan tasoon,
  // pois kuplan tieltä.
  { x: 8844, kuva: 'kuvat/piilotetut/storaenso-laatikko.png',
    lukitseKerrokseen: '7-middleground.png', syvyysSuhde: 0.722,
    yProsentti: 0.93, korkeusProsentti: 0.062, vainPysahdys: 8940 },

  // DNA - puhelin. Teksti oikealla -> puhelin vasemmalla, katutasossa.
  { x: 10405, kuva: 'kuvat/piilotetut/dna-puhelin.png',
    lukitseKerrokseen: '10-background.png', syvyysSuhde: 0.833,
    yProsentti: 0.90, korkeusProsentti: 0.07, vainPysahdys: 10470 },

  // UPM - tukkipino. UPM on listan tuorein asiakas, joten tämä on tietoisesti
  // yksi selkeimmin luettavista: hieman muita isompi ja lukittu katukerrokseen
  // niin että vain kaksi lähintä kerrosta peittää sitä.
  // Oli ensin arkkipino (upm-paperipino.png) - Jarno testasi 8.9.2026 ja
  // kysyi "What is UPM paper ream? What it represents?", eli esine ei
  // kertonut mitään. Vaihdettu tukkipinoon (Jarnon valinta), ks.
  // materiaalia/tee-piilotetut-esineet.py:n kommentti.
  { x: 11336, kuva: 'kuvat/piilotetut/upm-tukkipino.png',
    lukitseKerrokseen: '7-foreground-lane.png', syvyysSuhde: 0.5625,
    yProsentti: 0.92, korkeusProsentti: 0.075, vainPysahdys: 11415 },

  // ERICSSON - linkkimasto. Masto kuuluu kaupungin taivaanrantaan, ei
  // katutasoon. Teksti vasemmalla -> masto oikealla.
  //
  // KORJATTU 8.9.2026 (Jarno testasi: "Did not see Ericsson mast?").
  // Alkuperäinen sijoitus oli syvyysSuhde 0.3125 (= syvyys 2.5) eli
  // 5-middleground-buildings-kerroksen TAKANA, ja yProsentti 0.72 pudotti
  // maston keskelle rakennusvyöhykettä -> rakennukset peittivät sen
  // kokonaan. SAMA virhe kuin Finnairin koneella metsässä.
  // Nyt syvyys 3.5 = rakennuskerroksen ETEEN mutta katu- ja autokerrosten
  // (4-7) TAAKSE: masto ei voi enää jäädä piiloon MILLÄÄN ruutukoolla, ja
  // etualan liikenne kulkee silti sen jalustan editse, joten se pysyy
  // osana maisemaa eikä liimaudu päälle. Harkittiin myös maston jättämistä
  // rakennusten taakse niin että vain huippu näkyisi (kauniimpi), mutta se
  // olisi vaatinut tietoa siitä kuinka korkealle rakennukset piirtyvät -
  // ja se riippuu ruudun korkeudesta (taustat skaalataan ruutuun), joten
  // sama arvo ei olisi toiminut joka näytöllä.
  // lukitseKerrokseen vaihdettu samaan rakennuskerrokseen jonka tasossa
  // masto nyt seisoo. korkeusProsentti 0.17 -> 0.22 ja yProsentti 0.72 ->
  // 0.82: jalusta alemmas etualan taakse, huippu selvästi kattojen yli.
  { x: 11608, kuva: 'kuvat/piilotetut/ericsson-masto.png',
    lukitseKerrokseen: '5-middleground-buildings.png', syvyysSuhde: 0.4375,
    yProsentti: 0.82, korkeusProsentti: 0.22, vainPysahdys: 11485 },

  // METSÄ GROUP - pehmopaperirulla. Sama kohtaus kuin UPM/Ericsson mutta eri
  // pysähdys, joten nämä kolme eivät ole koskaan yhtä aikaa näkyvissä
  // (vainPysahdys). Teksti oikealla -> rulla vasemmalla.
  { x: 11427, kuva: 'kuvat/piilotetut/metsa-paperirulla.png',
    lukitseKerrokseen: '9-foreground-lane.png', syvyysSuhde: 0.8125,
    yProsentti: 0.94, korkeusProsentti: 0.062, vainPysahdys: 11490 },

  // DANSKE BANK - klassinen pankkirakennus kukkulan rinteessä. Teksti
  // oikealla -> rakennus vasemmalla.
  { x: 12312, kuva: 'kuvat/piilotetut/danske-pankki.png',
    lukitseKerrokseen: 'Hills Layer 04.png', syvyysSuhde: 0.5,
    yProsentti: 0.80, korkeusProsentti: 0.13, vainPysahdys: 12390 },

  // GIGANTTI - televisio. Teksti oikealla -> tv vasemmalla, rinteessä.
  { x: 12937, kuva: 'kuvat/piilotetut/gigantti-tv.png',
    lukitseKerrokseen: 'Hills Layer 05.png', syvyysSuhde: 0.7,
    yProsentti: 0.90, korkeusProsentti: 0.07, vainPysahdys: 12990 },

  // SANTANDER - pankkikortti. Tämän pysähdyksen teksti on KESKELLÄ
  // (puoli:'keski'), joten kortti menee reilusti oikealle ja alas.
  { x: 13354, kuva: 'kuvat/piilotetut/santander-kortti.png',
    lukitseKerrokseen: 'Hills Layer 05.png', syvyysSuhde: 0.7,
    yProsentti: 0.93, korkeusProsentti: 0.05, vainPysahdys: 13290 },
  ...
  // Paljastusdia juuri ennen Kiitos!-diaa: kaikki kymmenen esinettä yhdessä,
  // jotta Jarno voi kysyä yleisöltä huomasivatko he ne. Dia on tyhjä (ei
  // otsikkoa eikä tekstiä, ks. esitys-data.js x:13800) ja ketun animaatio on
  // 'Bark' - koko ruutu on siis vapaana tälle.
  //
  // TÄMÄ ON TARKOITUKSELLA kiintea:true eli RUUTUUN sidottu - päinvastoin
  // kuin yllä olevat kymmenen piiloesinettä, jotka ovat maailmaan
  // ankkuroituja. Ero on tahallinen: piiloversiot KUULUVAT maisemaan, tämä
  // on esittelytaulu. Ilman syvyysSuhde-kenttää kiintea-kuva menee
  // depth:200:aan eli KAIKEN eteen (Jarno: "you can bring it to front, all
  // of them") - ei siis voi jäädä minkään taustakerroksen taakse.
  // HUOM: tämä koskee VAIN tätä näyteikkunaa. Yllä olevien kymmenen
  // piiloesineen syvyydet jätettiin tarkoituksella ennalleen, jotta ne
  // pysyvät maiseman SISÄLLÄ eivätkä liimaudu sen päälle.
  //
  // Yksi koostekuva, ei kymmentä erillistä alkiota: asettelu (rivivälit,
  // keskitys, keskinäiset koot) on leivottu kuvaan, joten se pysyy
  // kohdallaan kaikilla ruutukooilla. Ks. materiaalia/tee-piilotetut-
  // esineet.py:n NÄYTEIKKUNA-lohko - siellä myös rivijako ja järjestys
  // (matalat ylös, korkeat alas; kummallakin rivillä esitysjärjestys
  // vasemmalta oikealle). SPRITESHEET siksi, että Siemensin monitorin
  // EKG-käyrä jatkaa sykkimistään myös näyteikkunassa.
  { x: 13800, spritesheet: 'kuvat/piilotetut/nayteikkuna.png',
    kehysLeveys: 187, kehysKorkeus: 98, kehyksia: 8, fps: 6,
    kiintea: true, puoli: 'keski', pysty: 'keski',
    korkeusProsentti: 0.45 },
];
...
const YLITYSHAHMOT = [
  ...
  // virtaa:true (9.9.2026, Jarno: "it goes once, can it repeate") - ilman
  // tätä Mech ylitti ruudun KERRAN ja jäi piiloon lopuksi asti, vaikka
  // pysähdyksellä viivyttäisiin pitkään (tyhjä, tekstitön hetki - voi
  // kestää kauan esim. yleisökeskustelussa). virtaa on jo olemassa oleva
  // kenttä (ks. update():n "virtaa"-kommentti, alun perin Nera-armeijalle
  // "several endless stream of she... until clicked to next") - laittaa
  // ylityksen aloittamaan HETI alusta uudelleen reunaan päästyään sen
  // sijaan että pysähtyisi, niin kauan kuin tällä pysähdyksellä ollaan.
